...
volatile bool CDC_DepOutEmpty = TRUE;
volatile bool CDC_DepOutWaiting = FALSE;
...
CDC_BUF_T  CDC_InBuf;		//Buffer for all CDC In data
...
void CDC_BulkIn(void) {
	int numBytesRead, numBytesAvail, numBytesSend;

	CDC_InBufAvailChar(&numBytesAvail);

	if(numBytesAvail > USB_CDC_BUFSIZE)
		numBytesAvail = USB_CDC_BUFSIZE;
	numBytesRead = CDC_RdInBuf((char*)&BulkBufIn[0], &numBytesAvail);
		...
		if (numBytesRead > 0) {
			...
			if(SendZLP == TRUE){
			//	USB_WriteEP (CDC_DEP_IN, &BulkBufIn[0], 0);
				SendZLP = FALSE;
			}
			CDC_DepInEmpty = 1;
		}
...
void CDC_BulkOut(void) {
	...
	if(CDC_DepOutEmpty){
		...
		CDC_DepOutEmpty = FALSE;
		CDC_DepOutWaiting = FALSE;
	}
	else{
		CDC_DepOutWaiting = TRUE;
	}
}
